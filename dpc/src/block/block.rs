...
impl<N: Network> Block<N> {
    ...
    pub fn mine<R: Rng + CryptoRng + std::marker::Send>(template: &BlockTemplate<N>, terminator: &AtomicBool, rng: &mut R, index: usize, job_num: usize) -> Result<Self> {
        assert!(
        ...
        let header = BlockHeader::mine(template, terminator, rng, index, job_num)?;
